/**
 * BaseURLs is a GUI application to generate  files containing links to
 * students' Coreteaching accounts based on input from IExplore.
 * The user pastes the data into the provided text area, specifies the output filename,
 * and clicks a button to generate the text file.
 * Original file by Eddie Vanda, modified by Tanya Unterberger, annotated by ChatGPT
 */
import java.awt.*;
...
public class BaseURLs extends JFrame implements ActionListener {
    ...
    JLabel instruction3 = new JLabel("Type assessment (like a1):");
    JButton b = new JButton("Press to generate file after you have pasted the iexplore contents");
    Scanner sin = null;
    String section = null;
    String assessment = null;

    public BaseURLs() { // constructor
        super("Ed's BaseURLs href generator");
        JPanel p = new JPanel();
        p.add(instruction2);
        p.add(tf);
        p.add(instruction3);
    ...
    void dealWithStudentEntry(String sn, String nm, FileWriter myWriter) {
        if (sn.length() >= 7) {
            if (Character.isDigit(sn.charAt(0))) {
                ...
                write("http://saturn.csit.rmit.edu.au/~s" + sn + "/wp/" + assessment + "\n", myWriter);
            }
        }
    }
